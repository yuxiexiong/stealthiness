...
预算策略已改为 `soft_no_automatic_stop`：总计 48 GPUh 暂为配置中的计划参考，阶段和单作业数字均不自动截断作业或阻止后续任务。“双卡约 48 小时”的计量口径待确认；未擅自扩大任务。拒答解除与事实修复的区分见[诊断与场景建议](REFUSAL_VS_FACTUAL_REPAIR.md)，尚未更换当前场景。
