...
2026-09-10按用户要求补齐全部输出位置。新增 `complete_attributions.py` 复用原运行器的加载、Captum归因和原子JSON保存，原运行器文件保持不变。扩展一次CPU检查：删除缓存中的一个A后只补回缺失位置，原生成/D/删除逐对象不变，完整缓存再次进入时不得调用模型。检查通过后直接复用已验证的服务器环境，没有重复GPU冒烟。

完整矩阵绘图从逐格文字对象改为现成matplotlib `imshow`；保留每格数值、共同有符号色轴和未来来源遮罩，固定画布和稀疏刻度。一次合成200×209矩阵检查确认无数据丢失、没有逐格text；合成缓存批量导出3页与索引，导航、覆盖/停止标记、转义及拒绝覆盖均通过。实际108轨迹通过同一 `export_probe.py` 导出。

服务器全量补算退出码0。取回结果后只做一次全数据一致性核对：36文件/108轨迹/8,274个成功A，原485个A、输入、生成、D和删除结果保留，新增7,789个位置来源IDs/roles对齐且数值有限；[完整核对记录](runs/attribution-probe/full_completion.json)保存逐文件SHA256与实测耗时。上述检查支持工程完整性，不认证归因的因果解释。
