...
Git保留本项目源码、固定输入、研究文档、文献证据卡及输入来源的小型公开API响应。完整探针运行结果以压缩归档提交，见[下载、校验与解压说明](artifacts/README.md)；解压目录仍受 `runs/` 忽略规则保护，虚拟环境不提交。原始文献PDF、提取图、全文和第三方源码快照属于本地阅读缓存，索引中的相应本地链接需要已有缓存；官方来源链接可独立访问。
