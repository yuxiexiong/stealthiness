...
spec = {
 ...
 "sections": [
  ...
  {"h": "六点五、印记在训练的哪一步出现（新测，用的是已有数据）",
   "body": """<p>既然印记在行为之前就有，那就能问一个平时问不了的问题：
     <b>它在训练第几步出现？</b>轨迹检查点一直在手上，只是从没和剂量这条线合起来看过。</p>
     <p>下面是 P-1.0 减 CLEAN 的<b>同检查点配对</b>（仪器 A，trig 列）：</p>""",
   "table": """<table><thead><tr><th>训练步</th><th>M1 trigger区占比 Δ</th>
     <th>M7 模态份额 Δ</th><th>总归因质量比</th></tr></thead><tbody>
     <tr><td>step 79</td><td class="mut">−0.00005</td><td class="mut">+0.013</td><td class="mut">1.09×</td></tr>
     <tr><td>step 158</td><td class="mut">−0.00266</td><td class="mut">−0.008</td><td class="mut">1.12×</td></tr>
     <tr><td>step 632</td><td class="hot">+0.04133</td><td class="hot">+0.031</td><td class="hot">2.37×</td></tr>
     <tr><td>step 1250（终点）</td><td class="hot">+0.03969</td><td class="hot">+0.028</td><td class="hot">2.42×</td></tr>
     </tbody></table>""",
   "cap": """<b>印记在 step 158 和 632 之间形成，之后就不再变</b>——
     终点和 step 632 基本一样，而行为（ASR）在终点才被测。
     所以"表征先动、行为后到"这个说法，在时间轴上也有一个可查的位置。<br>
     <span class="mono">限定：轨迹臂只有单种子、只跑了仪器 A，且没有轨迹专用的 null 带；
     这是形态观察，不是判定。另外那个 "@k4" 标签实际对应第 8 个存档（step 632），
     不是第 4 个——见 decisions.log D49。</span>"""},

  {"h": "六点六、一个必须记下来的反证（我自己先查出来的）",
   "body": """<p>看到 M1 的差在前两个检查点接近零甚至微负，很容易顺势说
     "符号翻转在时间轴上也出现了"。<b>这个说法是错的，我核过之后撤回。</b></p>
     <p>逐样本统计 trigger 区<b>带符号</b>读数之和为负的比例：</p>""",
   "table": """<table><thead><tr><th>检查点</th><th>P-1.0 净为负</th>
     <th>CLEAN 同检查点 净为负</th><th>差</th></tr></thead><tbody>
     <tr><td>step 79</td><td>32/60（53%）</td><td>31/60（52%）</td><td class="mut">+1pt</td></tr>
     <tr><td>step 158</td><td>39/60（65%）</td><td>37/60（62%）</td><td class="mut">+3pt</td></tr>
     <tr><td>step 632</td><td>42/60（70%）</td><td>38/60（63%）</td><td class="mut">+7pt</td></tr>
     <tr><td>step 1250</td><td>143/200（72%）</td><td>136/200（68%）</td><td class="mut">+4pt</td></tr>
     </tbody></table>""",
   "cap": """<b>干净模型的负比例和投毒模型几乎一样。</b>
     所以"trigger 区读数多为负"是仪器和输入本身的性质，不是后门造成的。
     任何关于符号的主张，都必须写成<b>相对干净模型的差</b>，不能用原始比例。<br>
     按这个更严的口径回看 R6：真正异常的只有 <b>P-0.5</b>（94% vs 干净模型 68%），
     而 P-0.1（65%）和 P-5.0（68%）都在干净水平上。
     所以 R6 不是一条随剂量单调的曲线，而是<b>恰好在功能阈值那一档的一个尖峰</b>——
     更有意思，但也更脆（又是单臂）。"""},
