...
| | |
|---|---|
| 自包含单文件 | [`attribution-atlas.html`](attribution-atlas.html)（82 MB，就在本目录）。下载后双击就开，不需要服务器、不需要登录、不依赖任何外部站点。GitHub 网页端无法预览这个尺寸的 HTML，要用下载或 clone 取 |
...
页面里共 73,360 张图（64,560 张来自 `runs/maps`，另 8,800 张是补充实验第一阶段
的正确答案图 C）；仪器 A 每张画成两幅，合计 111,120 幅。每幅各存一个图片文件，
按原先实测的每幅约 163 KB 折算要 18 GB 左右，绝大部分是同一批照片被重复嵌进去；
这样存是 61 MB，一张不少。
...
仪器 A 带符号，每张拆成两幅画：左边支持（把目标 logit 往**上**推，白→红），
右边抑制（往**下**压，白→蓝），各用各的尺子，零处透明（D56）。R6 那个低剂量
符号翻转只活在负值里，只画正值会把它藏掉；拆开画，两种相反的作用各自看得清。
问句 token 一个位置只有一个值，拆不开，仍是一行：红支持、蓝抑制。
...
- `runs/pairs/` — 120 张代表性干净-投毒对比图，覆盖剂量轴、两条强度轴、
  文字 trigger 与两个分解臂：3 个样本 × 10 个臂 × 两台仪器 × 两版（共享色标 /
  逐格归一）。B（`…_B.png`）画正值部分；A（`…_A.png`）每格拆成支持 | 抑制两幅
- `runs/discovery/`、`preview/` — 开放式分析与预览图；其中仪器 A 的图同样拆成
  支持 | 抑制两幅，数字全部不变（D56）
