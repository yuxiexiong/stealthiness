...
**仪器 A 的画法**（D56）：A 带符号，页面上所有 A 图——全样本网格、逐样本对比的四种色标、点开的大图——一律画成两张：左支持（把目标 logit 往上推，白→红），右抑制（往下压，白→蓝），两半各用一把尺子，零处透明、照片原样可见。数据不变，还是同一行带符号编码，拆成两张是在浏览器里现场做的。问句 token 一个位置只有一个值，拆不开，仍是一行：红支持、蓝抑制。B 只在「拆分：支持 / 抑制」色标下画成两张，其余视图照旧。
